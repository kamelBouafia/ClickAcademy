 ...
 * Globals
 */
var app, agent, credentials, user, lesson;
...
describe('Lesson CRUD tests', function() {
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/api/lessons')
					...
					.end(function(lessonSaveErr, lessonSaveRes) {
						...
						agent.put('/api/lessons/' + lessonSaveRes.body._id)
							.send(lesson)
							.expect(200)
	...
	it('should be able to get a single Lesson if not signed in', function(done) {
		...
		// Save the Lesson
		lessonObj.save(function() {
			request(app).get('/api/lessons/' + lessonObj._id)
				.end(function(req, res) {
					// Set assertion
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/api/lessons')
					...
					.end(function(lessonSaveErr, lessonSaveRes) {
						...
						agent.delete('/api/lessons/' + lessonSaveRes.body._id)
							.send(lesson)
							.expect(200)
	...
	it('should not be able to delete Lesson instance if not signed in', function(done) {
		...
		lessonObj.save(function() {
			// Try deleting Lesson
			request(app).delete('/api/lessons/' + lessonObj._id)
			.expect(403)
			.end(function(lessonDeleteErr, lessonDeleteRes) {
				// Set message assertion
				(lessonDeleteRes.body.message).should.match('User is not authorized');
